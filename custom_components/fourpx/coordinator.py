"""Coordinator for the 4PX parcel tracker integration.
...
from .api import FPXApiClient, FPXApiError
from .const import (
...
# Base for the 429 backoff when 4PX's response carries no ``Retry-After`` of
# its own: ``BACKOFF_BASE_SECONDS * 2**consecutive_429``, capped at
# ``BACKOFF_CAP_SECONDS``.
...
class FPXCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def __init__(
        ...
        # exponential backoff below. Reset to 0 on any success.
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (FPXApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if isinstance(result, FPXApiError) and result.status_code == 429:
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("4PX fetch failed; retaining any cached parcel data: %s", result.detail if isinstance(result, FPXApiError) else type(result).__name__)
        ...
        if saw_429:
            ...
            raise UpdateFailed("4PX rate-limited (429)", retry_after=retry_after)
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("4PX unreachable for all tracked parcels")
