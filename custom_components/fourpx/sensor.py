"""Sensor platform for the 4PX parcel tracker integration."""
...
from . import FPXConfigEntry
from .const import DOMAIN
from .coordinator import FPXCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: FPXConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up 4PX sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        FPXIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            FPXParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(FPXNextDeliverySensor(coordinator, entry))
    entities.append(FPXDeliveredParcelsSensor(coordinator, entry))
    entities.append(FPXLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class FPXIncomingParcelsSensor(
    CoordinatorEntity[FPXCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                FPXParcelSensor(self.coordinator, self._entry, barcode)
...
class FPXParcelSensor(CoordinatorEntity[FPXCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked 4PX parcel."""
    ...
    def __init__(
        self, coordinator: FPXCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class FPXNextDeliverySensor(
    ...
    def __init__(
        self, coordinator: FPXCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class FPXDeliveredParcelsSensor(
    CoordinatorEntity[FPXCoordinator], SensorEntity
):
    """Recently delivered tracked 4PX parcels."""
...
class FPXLastUpdateSensor(
    CoordinatorEntity[FPXCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when 4PX was last polled successfully."""
