"""Config flow for the 4PX parcel tracker integration."""
...
def normalize_tracking_code(value: str) -> str:
    """Strip surrounding whitespace and uppercase a 4PX tracking code."""
    return (value or "").strip().upper()


def valid_tracking_code(value: str) -> bool:
    """Accept every non-empty code; 4PX's possible formats are not settled."""
    return bool(value)
...
class FPXConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the 4PX integration."""
    ...
    ) -> FPXOptionsFlowHandler:
        """Return the options flow handler."""
        return FPXOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the 4PX hub — single instance, no input needed.
        ...
        the ``fourpx.track_parcel`` service or a dashboard button.
...
class FPXOptionsFlowHandler(OptionsFlow):
