"""Calendar platform for the 4PX parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the 4PX deliveries calendar from a config entry."""
    async_add_entities([FPXDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class FPXDeliveriesCalendar(CoordinatorEntity[FPXCoordinator], CalendarEntity):
    """A read-only calendar of expected 4PX deliveries.
    ...
    def __init__(self, coordinator: FPXCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "4PX parcel")
            description_parts = [
