"""4PX parcel tracker custom component for Home Assistant."""
...
@dataclass
class FPXData:
    """Runtime data attached to the 4PX config entry."""

    client: FPXApiClient
    coordinator: FPXCoordinator


type FPXConfigEntry = ConfigEntry[FPXData]


async def async_setup_entry(hass: HomeAssistant, entry: FPXConfigEntry) -> bool:
    """Set up 4PX from a config entry."""
    # No auth: 4PX tracking is public, so the HA-managed session is fine.
    client = FPXApiClient(async_get_clientsession(hass))
    coordinator = FPXCoordinator(hass, client, entry)
    ...
    entry.runtime_data = FPXData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: FPXConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: FPXConfigEntry) -> bool:
    """Unload the 4PX config entry."""
