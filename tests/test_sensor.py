"""Tests for 4PX sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.fourpx.const import ParcelStatus
from custom_components.fourpx.sensor import (
    FPXDeliveredParcelsSensor,
    FPXIncomingParcelsSensor,
    FPXLastUpdateSensor,
    FPXNextDeliverySensor,
    FPXParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = FPXIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = FPXParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = FPXParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = FPXNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = FPXNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = FPXDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = FPXLastUpdateSensor(coordinator, _entry())
