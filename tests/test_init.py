"""Tests for 4PX setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.fourpx.api.FPXApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.4px_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.fourpx.api.FPXApiClient.async_get_parcel",
        new=AsyncMock(side_effect=FPXApiError("4PX unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.fourpx.api.FPXApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
