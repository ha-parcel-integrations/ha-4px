"""Tests for the 4PX config and options flow."""

from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.fourpx.config_flow import (
...
def test_normalize_tracking_code_strips_surrounding_whitespace_and_uppercases():
    assert normalize_tracking_code("  synthetic-123  ") == "SYNTHETIC-123"
    assert normalize_tracking_code("") == ""
    assert normalize_tracking_code(None) == ""


def test_valid_tracking_code_is_nonempty_without_an_unproven_pattern():
    assert valid_tracking_code("SYNTHETIC")
...
async def test_options_preserves_internal_code_characters(hass):
    entry = _hub([])
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": [" synthetic-123 "]}
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [{CONF_TRACKING_CODE: "SYNTHETIC-123"}]
