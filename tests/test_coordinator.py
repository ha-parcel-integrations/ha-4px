"""Tests for the 4PX coordinator: fetching, caching and events.
...
from custom_components.fourpx.api import FPXApiError
...
from custom_components.fourpx.coordinator import FPXCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["tracks"][0]["tkCode"] = "FPX_C_AAF"
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = FPXApiError("HTTP 500")
...
async def test_update_backfills_missing_server_code(hass):
    """An edge payload without a server code keeps the requested code."""
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
    sample = active_sample()
    del sample["serverCode"]
    client = AsyncMock()
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = FPXCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"serverCode": "GONE"}
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
        # 4PX has no confirmed ETA field; therefore it never emits this event.
