...
- **Tiers while polling:** *hot* (15 min) when a tracked, not-yet-delivered
  ...
  `planned_from` at all — which is always true for 4PX, since it never
  populates `planned_from`); *mid* (45 min) for anything else still in
  flight — `problem`/`returning` included, deliberately not hot.
...
- **429 backoff:** a 429 anywhere in a poll raises `UpdateFailed` with
  `retry_after` — 4PX's own `Retry-After` header if present, otherwise an
  exponential backoff tracked per-coordinator. `api.py`'s
  `FPXApiError.status_code` / `.retry_after` carry this from the HTTP layer.
